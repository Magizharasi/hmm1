...
class BehavioralDriftDetector:
        ...
        for feature in common_features:
            try:
                ...
                if len(hist_values) > 5 and len(recent_values) > 5:
                    ...
                    if hist_range[1] > hist_range[0]:  # Avoid division by zero
                        ...
                        # Calculate JS divergence using our custom implementation
    ...
    def _calculate_overall_drift_score(self, statistical_tests: Dict[str, Any], 
        ...
        if psi_results:
            psi_scores = list(psi_results.values())
    ...
    def _classify_drift_type(self, user_id: int, timestamp: datetime) -> DriftType:
        ...
        earlier_records = history[-recent_window*2:-recent_window] if len(history) >= recent_window*2 else []
    ...
    def _calculate_drift_confidence(self, statistical_tests: Dict[str, Any], 
        ...
        sample_confidence = min(len(feature_drift_scores) / 20.0, 1.0) if feature_drift_scores else 0.5
    ...
    def _calculate_psi(self, baseline: pd.Series, current: pd.Series, bins: int = 10) -> float:
        ...
        try:
            ...
            try:
                ...
                bin_edges = np.linspace(baseline_clean.min(), baseline_clean.max(), bins + 1)
